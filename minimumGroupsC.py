...
control1 = tkobj.PhotoImage(file=".\\sample_images\\ctrl\\ctrl_1.png")
...
while procArrayCount < 7:  # Amount of Images is n - 1 = 6
    procList.append(tkobj.PhotoImage(file=".\\sample_images\\proc\\proc" + str(procArrayCount) + "_ctrl.png"))
    procArrayCount += 1
#####CHNG NMR
...
def loadScrn():
    ...
    if outroCount == 18:
        label.configure(image=control1)
        #outroCount +=1
    elif outroCount == 19:
        label.configure(image=wait)
        #outroCount += 1
    elif outroCount == 20:
        label.configure(image=control2)
        #outroCount += 1
    elif outroCount == 22:
        checkpoint = 3
...
def openChatplat(code):
    ...
    webbrowser.open(
        'https://unh.az1.qualtrics.com/jfe/form/SV_dm5HPyvVLz3chIV')
